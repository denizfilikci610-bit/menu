...
//   VISION_BASE_URL  = https://generativelanguage.googleapis.com/v1beta/openai  (valgfri — kun hvis du vil bruge en anden OpenAI-kompatibel vision-udbyder)
...
const SCENE_VISION_SYS = [
  'Du er en præcis LAYOUT-TRACER. Du får et referencebillede og skal gengive dets opbygning så NØJAGTIGT som muligt som ét JSON-objekt ("scene"), som motoren tegner præcist. Du redesigner IKKE og forbedrer IKKE — du KOPIERER referencens layout 1:1. Svar KUN med JSON.',
  '',
  ...
  '- icon:  {"t":"icon","icon":<id>,"fill":"#hex"}',
  '- path:  {"t":"path","d":<SVG path i 0..100 viewBox>,"fill":...,"stroke":"#hex","sw":<cqw>}',
  'FONT-ID (vælg den NÆRMESTE til referencens skrift): fraunces, playfair, dmserif (seriffer); anton, bebas, archivo, oswald (fede/smalle display); poppins, montserrat, grotesk (sans); caveat (håndskrift); mono2 (monospace).',
  'IKON-ID: coffee, food, leaf, flower, heart, gift, sun, sparkle, snowflake, confetti, scissors, dumbbell, graduation, sale, star.',
  '',
  ...
  '5. Læs farver direkte fra billedet (hex) og match dem.',
  ...
  '7. Brug så mange elementer som nødvendigt for at matche (intet kunstigt loft).',
  '8. Fyld tekstblokkene med brugerens egen tekst/brand hvor det passer (overskrift i overskrift-blokken osv.), men behold referencens placering og størrelse. Mangler du tekst, så skriv en kort passende tekst.',
  '',
  'Svar KUN med scene-JSON, ingen markdown, ingen forklaring.'
].join('\n');

async function callGeminiVision(key, model, baseUrl, messages, temp) {
  const url = String(baseUrl || '').replace(/\/+$/, '') + '/chat/completions';
  return fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key },
    body: JSON.stringify({ model, messages, temperature: (typeof temp === 'number' ? temp : 0.3), max_tokens: 5000 })
  });
}

// Genforsøg én gang ved forbigående overbelastning/kvote (503/429)
async function callGeminiVisionRetry(key, model, baseUrl, messages, temp) {
  let r = await callGeminiVision(key, model, baseUrl, messages, temp);
  if (r && (r.status === 503 || r.status === 429)) {
    await new Promise(res => setTimeout(res, 1300));
    r = await callGeminiVision(key, model, baseUrl, messages, temp);
  }
  return r;
}
...
async function handleSceneRequest(body, res, key) {
  ...
  const visionBase = process.env.VISION_BASE_URL || 'https://generativelanguage.googleapis.com/v1beta/openai';

  try {
    let parsed = null;
    let via = 'tekst';
    let visionNote = '';
    if (!visionKey) visionNote = 'ingen GEMINI_API_KEY på serveren';
    else if (!styleImage) visionNote = 'intet/ugyldigt billede modtaget (for stort?)';
    // 1) VISION-FØRST: referencebillede + vision-nøgle → lad en vision-model SE billedet og kopiere layoutet
    if (styleImage && visionKey) {
      ...
      try {
        const visSys = { role: 'system', content: SCENE_VISION_SYS };
        const visionUser = user + '\n\nKOPIÉR LAYOUTET fra det vedhæftede referencebillede så NØJAGTIGT som muligt. Sæt scene.ar til ' + ratio + ' (billedets forhold). Placér og størrelsessæt hvert element så det matcher billedet (x, y, w, h, fs), match farver og justering. Fyld tekstblokkene med brugerens egen tekst/brand herover, men behold referencens positioner og størrelser.' + jsonOnly;
        const vMessages = [ visSys, { role: 'user', content: [ { type: 'text', text: visionUser }, { type: 'image_url', image_url: { url: styleImage } } ] } ];
        const vr = await callGeminiVisionRetry(visionKey, visionModel, visionBase, vMessages, 0.15);
        if (vr.ok) {
          const vd = await vr.json();
          const vt = (vd && vd.choices && vd.choices[0] && vd.choices[0].message && vd.choices[0].message.content) || '';
          const vp = extractScene(typeof vt === 'string' ? vt : JSON.stringify(vt));
          if (vp.scene) { parsed = vp; via = 'vision'; visionNote = 'ok'; }
          else visionNote = 'Gemini svarede, men svaret kunne ikke læses som JSON';
        } else {
          visionNote = 'Gemini HTTP ' + vr.status;
        }
        ...
        if (parsed && parsed.scene && process.env.VISION_REFINE !== '0') {
          try {
            ...
            const refineUser = 'Her er dit forrige forsøg på at kopiere referencen som scene-JSON:\n' + prevJson + '\n\nSammenlign det NØJE med det vedhæftede referencebillede og RET alt der ikke matcher: elementernes position (x,y), størrelse (w,h,fs), justering, farver, baggrund — og tilføj manglende eller fjern overflødige elementer. Behold brugerens tekst i blokkene. scene.ar skal være ' + ratio + '.' + jsonOnly;
            const rMessages = [ { role: 'system', content: SCENE_VISION_SYS }, { role: 'user', content: [ { type: 'text', text: refineUser }, { type: 'image_url', image_url: { url: styleImage } } ] } ];
            const rr = await callGeminiVisionRetry(visionKey, visionModel, visionBase, rMessages, 0.1);
            if (rr.ok) {
              const rd = await rr.json();
              const rt = (rd && rd.choices && rd.choices[0] && rd.choices[0].message && rd.choices[0].message.content) || '';
              const rp = extractScene(typeof rt === 'string' ? rt : JSON.stringify(rt));
              if (rp.scene) { parsed = rp; via = 'vision+ret'; }
            }
