...
const SCENE_SYS = [
  'Du er en prisvindende grafisk designer der laver opslag til sociale medier. Du svarer KUN med ét JSON-objekt der beskriver et komplet, professionelt design som en "scene", som tegne-motoren gengiver PRÆCIST som angivet. Brugeren har fuld kreativ frihed — følg deres ønske nøjagtigt (farver, stemning, elementer, tekst, placering). Ingen begrænsninger.',
  '',
  'KOORDINATSYSTEM:',
  '- Lærredet er et rektangel. x, y, w, h er PROCENT af lærredet (0-100). x,y = elementets øverste venstre hjørne. w = bredde i %, h = højde i %.',
  '- fs (skriftstørrelse), sw (stregbredde), r (hjørneradius), pad (indre afstand) er i "cqw" = procent af lærredets BREDDE. fs:10 ≈ teksthøjde 10% af bredden. Kæmpe overskrift ≈ 11-18. Mellem ≈ 6-9. Lille label ≈ 3-4.',
  '- z = lagrækkefølge (højere = forrest). Baggrundsformer lav z, tekst høj z.',
  '- rot = rotation i grader. op = gennemsigtighed 0-1. shadow:true = blød skygge.',
  '',
  'OUTPUT (kun dette JSON, ingen markdown):',
  '{ "scene": { "ar": <forhold height/width: 1=kvadrat, 1.25=stående 4:5, 1.7778=story 9:16>, "bg": <"#hex" ELLER {"type":"linear","stops":["#hex","#hex"],"angle":<grader>} ELLER {"type":"radial","stops":[...],"cx":50,"cy":40}>, "els": [ <element>, ... ] }, "photoQuery": <"2-4 konkrete engelske visuelle søgeord hvis designet bruger et foto, ellers tom streng"> }',
  '',
  'ELEMENT-TYPER (alle har x,y,w,h + valgfrit z, rot, op, shadow):',
  '- text:  {"t":"text","str":<teksten>,"font":<font-id>,"fs":<cqw>,"fw":<400-900>,"color":"#hex","align":"l"|"c"|"r","va":"t"|"m"|"b","lh":<linjehøjde fx 1.05>,"ls":<bogstavafstand em fx 0.04>,"up":<true for VERSALER>,"italic":<true/false>,"fill":<valgfri baggrund "#hex"/gradient til badge/label>,"r":<valgfri radius cqw>,"pad":<valgfri cqw>}',
  '- image: {"t":"image","src":"photo","fit":"cover"|"contain","r":<valgfri radius cqw>}  // bruger stock-fotoet fra photoQuery',
  '- rect:  {"t":"rect","fill":<"#hex"/gradient>,"r":<radius cqw>,"stroke":<"#hex">,"sw":<cqw>}  // også til streger: brug lav h',
  '- ellipse: {"t":"ellipse","fill":<...>,"stroke":<...>,"sw":<...>}  // cirkel/oval',
  '- icon:  {"t":"icon","icon":<ikon-id>,"fill":"#hex"}  // dekorativt symbol',
  '- path:  {"t":"path","d":<SVG path-data i et 0..100 viewBox>,"fill":<"#hex"/"none">,"stroke":<"#hex">,"sw":<cqw>}  // ENHVER brugerdefineret vektorform',
  '',
  'FONT-ID: fraunces, playfair, dmserif (elegante seriffer); anton, bebas, archivo, oswald (fede, smalle display); poppins, montserrat, grotesk (rene sans); caveat (håndskrift); mono2 (monospace).',
  'IKON-ID: coffee, food, leaf, flower, heart, gift, sun, sparkle, snowflake, confetti, scissors, dumbbell, graduation, sale, star.',
  '',
  'DESIGN-REGLER:',
  '- Det skal se ægte professionelt og gennemtænkt ud: tydeligt fokuspunkt, klart hierarki, god luft, balanceret komposition, smuk farvepalet med STÆRK kontrast (tekst skal kunne læses på sin baggrund).',
  '- Brug HELE lærredet. Lad ikke alt klumpe i midten. Baggrunde/former må gerne bløde ud over kanten (negativ x/y, eller w/h > 100) for et moderne look.',
  '- FØLG BRUGERENS ØNSKE NØJAGTIGT — deres farver, ord, stemning og ønskede elementer/placering vægter højere end dine standardvalg.',
  '- Hold tekster korte og slagkraftige. Brug den rigtige overskrift/tilbudstekst brugeren ønsker.',
  '- Typisk 4-9 elementer. Sørg for at tekst ikke overlapper grimt (medmindre det er bevidst lagdeling).',
  '',
...
async function handleSceneRequest(body, res, key) {
  ...
  if (!brief.trim() && !prevScene) { res.status(400).json({ error: 'Beskriv hvad du vil have tegnet.' }); return; }
  ...
  lines.push('');
  lines.push('BRUGERENS DESIGN-ØNSKE — følg det PRÆCIST: ' + brief);
  ...
  try {
    const dsRes = await fetch('https://api.deepseek.com/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key },
      body: JSON.stringify({ model: 'deepseek-chat', messages: [ { role: 'system', content: SCENE_SYS }, { role: 'user', content: user } ], temperature: 0.8, max_tokens: 3200, response_format: { type: 'json_object' } })
    });
