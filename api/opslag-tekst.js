...
//   KV_REST_API_URL / KV_REST_API_TOKEN          (valgfri — rate-limit: max 5 AI-opslag pr. dag pr. besoeger.
//                                                 Opret i Vercel -> Storage -> KV (Upstash); den indsaetter selv disse to.
//                                                 Er de ikke sat, springes rate-limit over og endpointet koerer som foer.)
...
// ---- Rate-limit: max 5 AI-genereringer pr. dag pr. IP via Vercel KV / Upstash Redis (REST). ----
// Fail-open: er KV-nøglerne ikke sat (eller fejler kaldet), springes tjekket helt over, saa endpointet
// aldrig gaar i stykker. KV-noeglerne er HEMMELIGE og hoerer til som Environment Variables i Vercel — ALDRIG i frontend.
const RL_LIMIT = 5;
async function checkRateLimit(req) {
  const url = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) return { allow: true };                 // ikke konfigureret -> fail open
  try {
    const fwd = String(req.headers['x-forwarded-for'] || '');
    const ip = (req.headers['x-real-ip'] || fwd.split(',')[0] || '').trim() || 'ukendt';
    const day = new Date().toISOString().slice(0, 10);         // UTC-dato i noeglen -> nulstilles hvert doegn
    const key = 'gen:' + ip + ':' + day;
    const opts = { headers: { Authorization: 'Bearer ' + token } };
    const r = await fetch(url.replace(/\/$/, '') + '/incr/' + encodeURIComponent(key), opts);
    const j = await r.json();
    const count = Number(j && j.result);
    if (!Number.isFinite(count)) return { allow: true };       // uventet svar -> fail open
    if (count === 1) { try { await fetch(url.replace(/\/$/, '') + '/expire/' + encodeURIComponent(key) + '/172800', opts); } catch (e) {} }
    return { allow: count <= RL_LIMIT, count };
  } catch (e) { return { allow: true }; }                     // fejl -> fail open (bryd aldrig endpointet)
}
...
export default async function handler(req, res) {
  ...
  if (!rl.allow) { res.status(429).json({ error: 'Du har nået dagens grænse på ' + RL_LIMIT + ' gratis AI-opslag. Prøv igen i morgen.' }); return; }
