...
async function callGeminiNative(key, model, sysText, userText, imageDataUrl, temp) {
  ...
  const bodyObj = {
    ...
    generationConfig: { temperature: (typeof temp === 'number' ? temp : 0.2), maxOutputTokens: 8192, responseMimeType: 'application/json' }
  };
...
async function callGeminiNativeRetry(key, model, sysText, userText, imageDataUrl, temp) {
  let r = await callGeminiNative(key, model, sysText, userText, imageDataUrl, temp);
  if (r && (r.status === 503 || r.status === 429)) {
    await new Promise(res => setTimeout(res, 1300));
    r = await callGeminiNative(key, model, sysText, userText, imageDataUrl, temp);
  }
  return r;
}

// Samlet vision-kald. Standard = native Gemini (håndhævet JSON). Hvis VISION_BASE_URL er sat til en
// anden udbyder, bruges OpenAI-kompatibelt format i stedet. Returnerer { status, text }.
async function visionComplete(key, model, baseUrl, sysText, userText, imageDataUrl, temp) {
  ...
  if (!customBase) {
    ...
    if (!r || !r.ok) return { status: r ? r.status : 0, text: '' };
  ...
  return { status: 200, text: typeof c === 'string' ? c : JSON.stringify(c) };
}
    ...
    if (styleImage && visionKey) {
      ...
      try {
        ...
        if (r1.text) {
          ...
          visionNote = 'Gemini HTTP ' + r1.status;
        }
