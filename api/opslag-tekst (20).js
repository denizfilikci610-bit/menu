...
async function handleSceneRequest(body, res, key) {
  ...
  try {
    ...
    if (styleImage && visionKey) {
      try {
        ...
        if (vr.ok) {
          ...
          if (vp.scene) { parsed = vp; via = 'vision'; }
        }
        ...
        if (parsed && parsed.scene && process.env.VISION_REFINE !== '0') {
          try {
            ...
            const refineUser = 'Her er dit forrige forsøg på at kopiere referencen som scene-JSON:\n' + prevJson + '\n\nSammenlign det NØJE med det vedhæftede referencebillede og RET alt der ikke matcher: elementernes position (x,y), størrelse (w,h,fs), justering, farver, baggrund — og tilføj manglende eller fjern overflødige elementer. Behold brugerens tekst i blokkene. scene.ar skal være ' + ratio + '. Returnér KUN den rettede, komplette scene-JSON.';
            ...
            const rr = await callGeminiVision(visionKey, visionModel, visionBase, rMessages, 0.1);
            if (rr.ok) {
      ...
      } catch (e) { /* vision fejlede → fald tilbage til tekst nedenfor */ }
    }
    ...
    const result = { scene: parsed.scene, photos: [], photosHi: [], photoQuery: parsed.photoQuery, via };
