...
export default async function handler(req, res) {
  ...
  try {
    ...
    result.photos = await fetchPhotos(query);
    result.brand = brand;
...
async function fetchPhotos(query) {
  ...
  try {
    ...
    return (j.photos || []).map(p => p && p.src && (p.src.large2x || p.src.large || p.src.original)).filter(Boolean).slice(0, 12);
  } catch (e) { return []; }
}
...
function extractContent(content) {
  const out = { headline:'', emphasis:'', eyebrow:'', subline:'', badge:'', posts:[], design:{ palette:'', colors:null, motif:'', layout:'', font:'', style:'', photoQuery:'' }, photos:[], brand:{ used:false, host:'' } };
