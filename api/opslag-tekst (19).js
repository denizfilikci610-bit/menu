...
//   VISION_MODEL     = gemini-2.5-flash            (valgfri — kan sættes til fx gemini-3.5-flash hvis din konto har den)
...
async function callGeminiVision(key, model, baseUrl, messages) {
...
async function handleSceneRequest(body, res, key) {
  ...
  try {
    ...
    if (styleImage && visionKey) {
      try {
        const visionUser = user + '\n\nLAYOUT-KOPI (VIGTIGST): Et referencebillede er vedhæftet. Se NØJE på dets opbygning og genskab den NØJAGTIGE komposition som en scene: samme placering, størrelse og proportioner for foto-/billedområde, tekstblokke, farveblokke, badges, streger og grafik — samme justering, rækkefølge og visuelle hierarki. Sæt elementernes x/y/w/h så de matcher referencen så tæt som muligt. Udskift KUN selve indholdet med brugerens tekst, brand og palet herover; behold referencens LAYOUT. Ignorér dine egne layout-vaner og kopiér billedet. Returnér KUN scene-JSON.';
        const vMessages = [ sysMsg, { role: 'user', content: [ { type: 'text', text: visionUser }, { type: 'image_url', image_url: { url: styleImage } } ] } ];
        const vr = await callGeminiVision(visionKey, visionModel, visionBase, vMessages);
        if (vr.ok) {
