...
export default async function handler(req, res) {
  ...
  if (usageUserId) {
    ...
    if (usageUsed >= 0 && usageUsed >= MONTHLY_LIMIT) {
      res.status(429).json({ error: 'Du har brugt dine ' + MONTHLY_LIMIT + ' gratis opslag denne måned. Der kommer nye den 1. — eller opgrader snart for ubegrænset.', limitReached: true, usage: { used: usageUsed, limit: MONTHLY_LIMIT, remaining: 0 } });
      return;
    }
  }
  ...
  try {
    ...
    if (usageUserId && usageUsed >= 0) {               // tael KUN vellykkede genereringer, kun login-brugere
      ...
      result.usage = { used: usageUsed, limit: MONTHLY_LIMIT, remaining: Math.max(0, MONTHLY_LIMIT - usageUsed) };
    }
